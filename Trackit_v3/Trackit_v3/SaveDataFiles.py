...
def SaveDataToFile(events, inputs, meanAccuracy, stdAccuracy, meanTimeOnTarget, stdTimeOnTarget, totalTimeOnTargets, dpg):
    ...
    with open('{}.txt'.format(file_name), 'w') as r:
        r.write("Rect_no\tEventType\tTrigger\tHeight\tPosition\ttotal Time visible\t Visible From" +
                "\tEntry_time (ms)\tTime_on_target (ms)\tSD outside Target(px)\tMean Inaccuracy (px outside target)\tExit_time (ms)"+
                "\tPercent_Time_On_Target (%)\tReaction_Time (ms)\ttime_Off_Target (ms)\t overshoot?"+
                "\t overshoot time\t undershoot?\t undershoot time\n")
    ...
    with open('{}.txt'.format(file_name_inputdata), 'w') as r:
        r.write("Digital_Input\tScreen Position Y axis (px)\ttime (ms)\n")
    ...
    with open('{}.txt'.format(file_name_statistic), 'w') as r:
        r.write("Mean Accuracy\t Std Accuracy\tMean Time On Target\t Std Time On Target\t Total Time On Target\n")
