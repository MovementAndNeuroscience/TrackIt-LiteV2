...
def CalculateOverAndUndershoot(events):
    ...
    for i, event in enumerate(events):
        ...
        if events[i].eventType == 'R':
            for j, input in enumerate(events[i].inputDuringEvent.inputdatas):
                if j+1 < len(events[i].inputDuringEvent.inputdatas) and j+2 < len(events[i].inputDuringEvent.inputdatas) and j+3 < len(events[i].inputDuringEvent.inputdatas) and j-1 > 0 and events[i].inputDuringEvent.inputdatas[j].screenPosY > 100:
