...
rects_list = filter(lambda x: x.startswith("R"), rects_pauses_list)
rect_h0s = [int(r_str[1:4]) for r_str in rects_list]
total_num_rects = len(filter(lambda x: x.startswith("R"), rects_pauses_list))
...
trace_size = 4
ready = False  # a flag for strarting after pressing ENTER`
...
def refresh_background():
    global time_ref, cur_event, is_rect_now, rect_h0, time_pause_s, already_on_target, cur_rect, was_on_target, time_on_target, time_on_target_ref, exit_time_ref
    ...
    if cur_event == "" or is_rect_now and time.time() - time_ref >= time_rect_s or not is_rect_now and time.time() - time_ref >= time_pause_s:
        ...
        if cur_event != "": # not the beginning
            if cur_event[0] == "R": # and it is a rect now
...
# take the sample and update the moving average (move 0,1,2,3 forwards and then overwrite 0)
...
while not should_quit:
    ...
    if not ready:  # wait until ENTER pressed
        ...
        if len(rects_pauses_list) > 0:
            ...
            #print "Current: {}".format(time.time() - exit_time_ref)
        else:
